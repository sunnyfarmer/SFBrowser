...
public class HistoryExpandableListAdapter extends BaseExpandableListAdapter {
	...
	public HistoryExpandableListAdapter(Context context, OnCheckedChangeListener bookmarksChangeListener, Cursor cursor, int dateIndex, int faviconSize) {
		...
		mIdIndex = cursor.getColumnIndex(BaseColumns._ID);
			...
			while (!mCursor.isAfterLast()) {
				...
				if (index > dateIndex) {
					...
					if (index == DateSorter.DAY_COUNT - 1) {
						// We are alresy int the last bin, so it will include all the remaining items.
	...
	/**
	 * Translates from a group positon in the ExpandableList to a bin. This is
	...
	public Object getChild(int groupPosition, int childPosition) {
		...
		return new HistoryItem(
				mCursor.getLong(mCursor.getColumnIndex(Browser.BookmarkColumns._ID)),
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		HistoryItem item = (HistoryItem) this.getChild(groupPosition, childPosition);
