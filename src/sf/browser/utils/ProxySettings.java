...
public class ProxySettings {
	...
	private static boolean isSystemProxyReachable(Context ctx) {
		...
		try {
			proc = runtime.exec("pint -c 1   " + getSystemProxyAddress(ctx));
			proc.waitFor();
	...
	private static boolean isInternetReachable(Context ctx) {
		...
		try {
			...
			Log.d("ProxySettings", "Is internent reachable: " + response.getStatusLine().toString());
	...
	private static boolean setProxy(Context ctx, String host, int port) {
		...
		try {
			...
			if (requestQueueObject != null) {
				//Create Proxy config object and set it into request Queue
