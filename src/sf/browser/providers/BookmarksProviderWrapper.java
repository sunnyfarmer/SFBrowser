...
import sf.browser.model.UrlSuggestionItemComparator;
...
public class BookmarksProviderWrapper {
	...
	public static List<BookmarkItem> getStockBookmarksWithLimit(ContentResolver contentResolver, int limit) {
		...
		String[] columns = new String[] {Browser.BookmarkColumns._ID, Browser.BookmarkColumns.TITLE, Browser.BookmarkColumns.URL, Browser.BookmarkColumns.FAVICON};
		
		Cursor cursor = contentResolver.query(BOOKMARKS_URI, columns, whereClause, null, orderClause);
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				...
				while (!cursor.isAfterLast() && count < limit) {
					BookmarkItem item = new BookmarkItem(cursor.getString(columnTitle), cursor.getString(columnUrl));
	...
	public static void deleteStrockBookmark(ContentResolver contentResolver, long id) {
		String whereClause = Browser.BookmarkColumns._ID + " = " + id;
			...
			if (c.moveToFirst()) {
				if (c.getInt(c.getColumnIndex(Browser.BookmarkColumns.BOOKMARK))==1) {
					if (c.getInt(c.getColumnIndex(Browser.BookmarkColumns.VISITS)) > 0) {
						...
						// nerver visited, it can be deleted.
	...
	/**
	 * Modify a bookmark/history record. If an id is provided, it look for it and update its vlaues. If not, values will be inserted.
	...
	public static void setAsBookmark(ContentResolver contentResolver, long id, String title, String url, boolean isBookmark) {
		boolean bookmarkExist = true;
		
		if (id != -1) {
			String[] columns = new String[] {Browser.BookmarkColumns._ID};
			String whereClause = Browser.BookmarkColumns._ID + " = " + id;
			
			Cursor cursor = contentResolver.query(BOOKMARKS_URI, columns, whereClause, null, null);
	...
	/**
	 * Delete an history record, e.g. reset the visited count and visited date if it is a bookmark, or delete it if not.
	 * @param contentResolver
	 * @param id
	 */
	...
	public static void updateHistory(ContentResolver contentResolver, String title, String url, String originalUrl) {
		String[] columns = new String[] {Browser.BookmarkColumns._ID, Browser.BookmarkColumns.URL, Browser.BookmarkColumns.BOOKMARK, Browser.BookmarkColumns.VISITS};
		String whereClause = Browser.BookmarkColumns.URL + " = \"" + "\" OR " + Browser.BookmarkColumns.URL + " = \"" + originalUrl + "\"";
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				...
				// If it is not a bookmark, we can update the title. If we were doing it on bookmarks, we would override the title choosen by the user.
				if (cursor.getInt(cursor.getColumnIndex(Browser.BookmarkColumns.BOOKMARK)) != 1) {
					values.put(Browser.BookmarkColumns.TITLE, title);
				}
...
//		BitmapDrawable icon = ApplicationUtils.getNormalizedFaviconForBookmarks(currentActivity, favicon);
		...
		ContentValues values = new ContentValues();
		values.put(BookmarkColumns.FAVICON, os.toByteArray());
	...
	public static void clearHistoryAndOrBookmarks(ContentResolver contentResolver, boolean clearHistory, boolean clearBookmarks) {
		if (!clearBookmarks && !clearHistory) {
			return;
		}
